function calculadoraDerivadaIntegral() {
    ...
    function separarFuncao(funcaoOriginal) {
      ...
      termos.push(termoAtual.replace(/\s/g, '')); // Adiciona o último 'termoAtual' (removendo espaços internos) ao array 'termos' após o loop terminar.
    ...
    let coeficiente = parseFloat(sinal + (coeficienteStr === '' ? '1' : coeficienteStr));
    if (isNaN(coeficiente)) coeficiente = sinal === '-' ? -1 : 1;
